...
export default async function AbsentPage({
  ...
  if (sheetId) {
    ...
    const schoolDaysList = countSchoolDays(startStr, end, calendar?.holidays || new Set(), calendar?.schoolOpenDays || new Set())
    const schoolDays = schoolDaysList.length
    ...
    absentStats = filteredStudents.map(st => {
      ...
      const daysWithRecord = new Set(recs.map(r => r.timestamp.slice(0,10))).size
  ...
  return (
    <>
      ...
      {session.role !== 'teacher' && (
        <GradeRoomFilter grades={grades} rooms={rooms} selGrade={selGrade} selRoom={selRoom} basePath="/admin/absent" />
      )}
